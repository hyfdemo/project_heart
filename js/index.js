...
function navPosition(index){
  $(window).ready(function(){
    $(index.concat('-link')).click(function(e){
      e.preventDefault();
      $('body').animate({scrollTop:$(index.concat('-box')).offset().top-50},500);
    })
  })
}
navPosition('#car');
...
$('.downArrow').click(function(e){
    e.preventDefault();
    $('body').animate({scrollTop:$('#car-box').offset().top-50},500);
});
...
$.ajax({
  ...
  success:function(data){
   ...
   $('#ban .carousel-inner').html(html);
   $('#ban .carousel-inner .item.bm').addClass('active');
    $('#ban .item').css({
      width:$(window).width(),
    ...
    if($('#ban .item').hasClass('active')) {
      $('#ban .item').find('h1').addClass('animated fadeInDownBig');
      $('#ban .item').find('h4:first').addClass('animated-fast fadeInLeft delay-1').next('h4').addClass('animated-fast fadeInRight delay');
    }
  }
});
    ...
    for(var i=0;i<data.length;i++){
      html+=`        <div class="col-xs-12 col-sm-4">
          <div class="pro-caption text-center" >
            <span class="glyphicon glyphicon-user"></span> ${data[i].pAuthor}
            <h4>${data[i].pTitle}</h4>
